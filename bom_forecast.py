...
SENSOR_TYPES = {
    'temperature': ['air_temp', 'Current Temp °C', TEMP_CELSIUS, 'mdi:thermometer'],
    ...
    if friendly:
        ...
        for index in range(start, config.get(CONF_DAYS)+1):
            for condition in monitored_conditions:
                add_entities([BOMForecastSensor(bom_forecast_data, condition, index, name, product_id)])
...
class BOMForecastSensor(Entity):
    ...
    def state(self):
        ...
        reading = self._bom_forecast_data.get_reading(self._condition, self._index)
            
        """Update the min and max temperatures throughout the day because BOM don't always report the max and min temp for the current day"""
        if ('n/a' in reading):
            current_temp = self._bom_forecast_data.get_reading('temperature', 0)
            if self._condition == 'min' and self._index == 0 and current_temp != 'n/a':
                if int(current_temp) < TODAYS_MIN:
                    TODAYS_MIN = int(current_temp)
                    reading.replace('n/a', TODAYS_MIN)
            if self._condition == 'max' and self._index == 0 and current_temp != 'n/a':
                if int(current_temp) < TODAYS_MAX:
                    TODAYS_MAX = int(current_temp)
                    reading.replace('n/a', TODAYS_MAX)
    ...
    def device_state_attributes(self):
        ...
        """Reset TODAYS_MIN and TODAYS_MAX each day"""
        current_date = datetime.datetime.strptime(datetime.datetime.now(), "%Y-%m-%dT%H%M%S%z")
        global LAST_DATE
        global TODAYS_MIN
        global TODAYS_MAX
...
class BOMForecastSensorFriendly(Entity):
    ...
    def state(self):
        ...
        for condition in self._conditions:
            reading = self._bom_forecast_data.get_reading(condition, self._index)
            """Update the min and max temperatures throughout the day because BOM don't always report the max and min temp for the current day"""
            if ('n/a' in reading):
                current_temp = self._bom_forecast_data.get_reading('temperature', 0)
                if condition == 'min' and self._index == 0 and current_temp != 'n/a':
                    global TODAYS_MIN
                    if int(current_temp) < TODAYS_MIN:
                        TODAYS_MIN = int(current_temp)
                        reading.replace('n/a', TODAYS_MIN)
                if condition == 'max' and self._index == 0 and current_temp != 'n/a':
                    global TODAYS_MAX
                    if int(current_temp) < TODAYS_MAX:
                        TODAYS_MAX = int(current_temp)
                        reading.replace('n/a', TODAYS_MAX)
            friendly_state = friendly_state.replace('{{{}}}'.format(condition), reading)
